...
engine.audio.effect.createFeedbackDelay = ({
...
engine.audio.effect.createMultitapFeedbackDelay = ({
  dry: dryAmount = 1,
  tap: tapParams = [],
  wet: wetAmount = 0.5,
} = {}) => {
  ...
  return {
    ...
    transition: function (taps = [], duration) {
      this.param.tap.forEach((tap, i) => {
        if (!taps[i]) {
          engine.utility.ramp.linear(tap.gain, engine.const.zeroGain, duration)
          return
        }
        ...
        if (typeof delay != 'undefined') {
          engine.utility.ramp.linear(tap.delay, delay, duration)
        }

        if (typeof feedback != 'undefined') {
          engine.utility.ramp.linear(tap.feedback, feedback, duration)
        }

        if (typeof gain != 'undefined') {
          engine.utility.ramp.linear(tap.gain, gain, duration)
        }
      })
...
engine.audio.effect.createPhaser = ({
  ...
  feedback: feedbackAmount = engine.const.zeroGain,
  rate: rateAmount = 1,
  type = 'sine',
  wet: wetAmount = 1/2,
  when = 0,
...
engine.audio.effect.createPingPongDelay = function (options) {
  const context = engine.audio.context(),
    feedbackDelay = this.createFeedbackDelay(options),
